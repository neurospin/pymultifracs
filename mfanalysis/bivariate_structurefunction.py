...
class BiStructureFunction:
    """
    This class provides methods for computing and analyzing bivariate struture functions S(j, q1, q2)
    ...
    Args:
        ...
        values (numpy.array)         :   values[ind_q1, ind_q2, ind_j] = values of S(j, q1, q2), with q_i = self.q_i[ind_qi]
                                         and j = self.j[ind_j]

        logvalues (numpy.array)      :  logvalues[ind_q1, ind_q2, ind_j] = values of log_2 (S(j, q1, q2)),
    ...
    def __init__(self, mrq_1, mrq_2, q_1, q_2, j1, j2, wtype):
        ...
        self.j1        = j1
        self.j2        = j2
        self.j         = np.array(list(mrq_1.values)) # should be the same as np.array(list(mrq_2.values))
        self.wtype     = wtype
    ...
    def plot(self, figlabel_structure = None, figlabel_scaling = None):
        ...
        fig.suptitle(self.mrq_1.name + ' - bivariate structure functions $\log_2(S(j,q_1, q_2))$')
        ...
        for ind_q1, q1 in enumerate(self.q_1):
            for ind_q2, q2 in enumerate(self.q_2):
                ...
                ax.plot(x, y, 'r--.')
                ax.set_xlabel('j')
                ax.set_title('$q_1$ = %0.1f,  $q_2$ = %0.1f'%(q1, q2) )
                ax.grid()
                plt.draw()
        ...
        if nq > 1:
            ...
            surf = ax.plot_surface(Q1, Q2, self.zeta, cmap=cm.coolwarm, linewidth=0, antialiased=False)
            ax.set_xlabel('$q_1$')
            ax.set_ylabel('$q_2$')
            ax.set_zlabel('$\zeta(q_1, q_2)$')
