function [coef, nj] = TxMFDFA_1d(data, poly_ord, p)
%
...
for j = 1 : j2
    ...
    ncoefs = floor (length (data) / (2 ^ j));
    nj.L(jid) = ncoefs;  % FIXME ¿y si j1 > 1?
    nj.W(jid) = ncoefs;
